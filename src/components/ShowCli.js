...
class ShowCli extends Component {
  ...
  showCli(resolve) {
    ...
    axios({
      method: "post",
      url: process.env.REACT_APP_PYNETWORK_URL + "/inventory/show/cisco/",
        ...
        <Grid container>
          <Grid item ms={4} style={this.tableStyle}>
            <form noValidate autoComplete="off">
              <TextField
                label="Show Commands in seperate lines"
                fullWidth
                variant="outlined"
                id="showCLIinput"
                minRows="5"
          ...
          <Grid item ms={8}>
            {this.setIssucess && (
              <table border="1">
                <tbody>
                  {Object.keys(this.SucessMessages).map(
                    (val, index) => (
                      <tr key={index}>
                        <td>{val}</td>
                        <td>
